...
export async function readTypesCache(): Promise<string[]> {
  try {
    ...
    if (!Array.isArray(parsed) || !parsed.every((item) => typeof item === 'string')) {
      console.warn('moderate-service types cache file is invalid JSON array');
      return [];
    }
    return parsed;
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.warn(`moderate-service types cache unavailable: ${message}`);
    return [];
  }
}
...
export async function upsertTypeInCache(typeName: string): Promise<void> {
  ...
  const current = await readTypesCache();
  if (current.some((existing) => normalize(existing) === normalize(trimmed))) {
    return;
  }

  const next = [...current, trimmed].sort((a, b) => a.localeCompare(b));
  await writeTypesCacheAtomically(next);
  console.log(`moderate-service cached new type from event: ${trimmed}`);
}
